...
- [skills-install](skills/skills-install/SKILL.md) — 第三方技能安装（仅显式调用）：内置预设一键装知名技能（grill-me 一组四件、ui-ux-pro-max 一组七件）；预设外让用户给官方地址，只用网页抓取取证（禁网络搜索、禁 curl），防 prompt 注入。
