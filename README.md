...
Agent 随身工具箱：跨项目可复用的技能（skills）、subagent 定义（agents）与一键安装脚本（python）。
...
`agents/<name>.md`，YAML frontmatter 必填 `name` 与 `description`，Markdown 正文即系统提示词。
...
├── agents/       # subagent 定义：每个文件一个 <name>.md（Claude Code 格式）
...
- [rust-web-engineer](agents/rust-web-engineer.md) — Rust + Web 前端全栈工程师：涉及 Rust（tokio / wasm / axum）、Web UI（TypeScript / React / Vue）或两者集成（Tauri / WASM / REST）的任务主动委派给它。
...
技能根目录，改完即时生效）；`agents/` 按目标工具**转换格式后写入**各工具的 agents
目录（改源文件后重跑安装命令同步，status 会提示内容漂移）。脚本纯标准库、跨平台，
不依赖平台特有 shell；按自身位置定位仓库，任何目录可执行：

```bash
python scripts/install.py                    # 一键：全部技能 + 全部 agent（幂等，默认命令）
python scripts/install.py skill git-finish   # 只装指定技能
python scripts/install.py agent rust-web-engineer  # 只装指定 agent
python scripts/install.py agent --to codex,gemini  # agent 装到指定目标
...
- agents 目标（`--to`，可重复、可逗号分隔、`all` = 全部，默认 `claude,zcode`）：
...
- flags 只有三个：`-n` 预览不改动；`--to` 指定 agent 目标；`--force` 卸载兜底
  （`status`/`remove` 可加 `skill` 或 `agent` 只处理一类）。
- 目标处若已是指向本仓库的技能链接 → 跳过；指向别处的旧链接 → 重建；
  agents 只覆盖本脚本生成的同名文件，**实体目录/他人文件一律不动**。
...
- subagent：真源在本仓库 `agents/`（Claude Code 格式），各工具目录里是**转换后的副本**，
  改源文件后重跑 `python scripts/install.py` 同步；Cursor 另会直接读 `.claude/agents`。
  私有 agent 放 `agents/x-*.md`（`.gitignore` 已排除），同样参与分发。
...
- `model` 省略时用各工具默认模型；Codex 目标按官方 agent role 校验（未知键会拒绝），
  转换器只输出其合法键。
- 私有 subagent 文件名用 `x-` 前缀（`.gitignore` 已排除 `agents/x-*`）。
